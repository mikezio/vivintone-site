...
function App() {
  ...
  if (!user) return <div className="auth-screen"><Container header={<Header variant="h1">VivintOne project admin</Header>}><SpaceBetween size="l"><p>This private workspace is only for the project maintainer. Sign-in uses a short-lived authorization-code flow with PKCE.</p><Flashbar items={flashes} /><Button variant="primary" disabled={!manager || !config} onClick={() => manager.signinRedirect()}>Sign in</Button><Link href="/">Return to the project site</Link></SpaceBetween></Container></div>;
  ...
  return <AppLayout
    navigation={<SideNavigation header={{ href: '/', text: 'VivintOne project admin' }} activeHref={`#${section}`} onFollow={event => { event.preventDefault(); const next = event.detail.href.slice(1); location.hash = next; setSection(next); }} items={[
